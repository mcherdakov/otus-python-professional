...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        pred_one = 1 / (1 + np.exp(-X@self.w))
        y_proba = np.vstack((1 - pred_one, pred_one)).T
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        # first calculating hypothesis in respect of current weights
        hyp = 1 / (1 + np.exp(-X_batch@self.w))

        # now calculate loss as sum of cost fuctions
        loss = np.transpose(-y_batch)@np.log(hyp) - np.transpose(1 - y_batch)@np.log(1 - hyp)

        # calculate gradient as first derivative of logistic loss function

        dw = X_batch.T@((1 / (1 + np.exp(-X_batch@self.w)) - y_batch))

        # Right now the loss is a sum over all training examples, but we want it
        # to be an average instead so we divide by num_train.
        # Note that the same thing must be done with gradient.

        loss = (1 / y_batch.size) * loss
        dw = (1 / y_batch.size) * dw

        # Add regularization to the loss and gradient.
        # Note that you have to exclude bias term in regularization.

        loss = loss + (reg / (2 * y_batch.size)) * self.w[1:].T@self.w[1:]
        dw = dw + (reg / y_batch.size) * self.w
